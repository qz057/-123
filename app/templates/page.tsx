    ...
    <div className="fd-page-chrome mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-9 lg:px-8">
      <header className="mb-8 grid gap-5 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-start">
        ...
        <Card className="rounded-[28px] border border-slate-200 bg-indigo-950 py-0 text-white shadow-sm fd-dark-card">
          <CardHeader>
            <p className="text-xs font-medium uppercase tracking-[0.18em] text-slate-500">Template map</p>
      ...
      <section className="mb-8 sm:hidden">
        <Card className="rounded-[28px] border border-slate-200 bg-indigo-50/70 py-0 shadow-sm fd-soft-card">
          <CardHeader>
            <p className="text-xs font-medium uppercase tracking-[0.18em] text-slate-500">Mobile pick guide</p>
          ...
          <CardContent className="space-y-3 pb-5 text-sm leading-[1.65] text-slate-600">
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-2.5 pt-1">
              <Link href="/diagnose" className="inline-flex rounded-full bg-indigo-950 fd-primary-glow px-4 py-2 text-sm font-medium text-white transition hover:bg-violet-900">
                先去 Diagnose
              </Link>
      ...
      <section className="mb-8 hidden sm:block">
        <div className="mb-4">
          <p className="text-xs font-medium uppercase tracking-[0.18em] text-slate-500">Entry routes</p>
          <h2 className="mt-1 text-xl font-semibold text-slate-950">现在最常见的三种进模板方式</h2>
        </div>
        <div className="grid gap-3.5 lg:grid-cols-3">
          {entryRoutes.map((item, index) => (
            <Card key={item.title} className="rounded-[28px] border border-slate-200 bg-white py-0 shadow-sm fd-glass-card">
              <CardHeader>
                <p className="text-xs font-medium uppercase tracking-[0.18em] text-slate-500">Route 0{index + 1}</p>
              ...
              <CardContent className="space-y-3.5 pb-5">
                ...
                <div className="flex flex-wrap items-center gap-x-4 gap-y-2.5">
                  <Link href={item.primary.href} className="inline-flex rounded-full bg-indigo-950 fd-primary-glow px-4 py-2 text-sm font-medium text-white transition hover:bg-violet-900">
                    {item.primary.label}
                  </Link>
      ...
      <section className="mb-7 hidden sm:block">
        <div className="mb-3.5">
          <p className="text-xs font-medium uppercase tracking-[0.18em] text-slate-500">Decision points</p>
          <h2 className="mt-1 text-xl font-semibold text-slate-950">这页真正帮你决定的，不只是“点哪个模板”</h2>
        </div>
        <div className="grid gap-3 lg:grid-cols-3">
          {decisionPoints.map((item) => (
            <Card key={item.title} className="rounded-[24px] border border-slate-200 bg-indigo-50/70 py-0 shadow-sm fd-soft-card">
      ...
      <section id="template-rules" className="mb-8 hidden gap-3.5 xl:grid-cols-[1.04fr_0.96fr] sm:grid">
        <Card className="rounded-[28px] border border-slate-200 bg-indigo-50/70 py-0 shadow-sm fd-soft-card">
          <CardHeader>
            <p className="text-xs font-medium uppercase tracking-[0.18em] text-slate-500">Selection rules</p>
        ...
        <Card className="rounded-[28px] border border-slate-200 bg-white py-0 shadow-sm fd-glass-card">
          <CardHeader>
            <p className="text-xs font-medium uppercase tracking-[0.18em] text-slate-500">Category map</p>
          ...
          <CardContent className="space-y-4 pb-5">
            ...
            <div className="grid gap-2.5 sm:grid-cols-3 lg:grid-cols-1">
              {difficultyStats.map((difficulty) => (
                <div key={difficulty} className="rounded-2xl border border-slate-200 bg-indigo-50/80 px-3.5 py-2.5 text-sm text-slate-600 shadow-[0_20px_52px_-40px_rgba(79,70,229,0.24)]">
      ...
      <section id="template-grid">
        <div className="mb-4 flex items-center justify-between gap-4">
          ...
          <Link href="/diagnose" className="hidden text-sm font-medium text-indigo-700 sm:inline-flex">
            不知道选哪个？先体检配置
          </Link>
        </div>
          ...
          {templatesCatalog.map((template, index) => (
            <Card key={template.slug} className="rounded-[28px] border border-slate-200 bg-white py-0 shadow-sm fd-glass-card">
              ...
              <CardContent className="space-y-3 pb-4">
                ...
                <div className="hidden rounded-2xl border border-slate-200 bg-indigo-50/65 px-3.5 py-3 sm:block">
                ...
                <div className="hidden flex-wrap gap-2 text-xs text-slate-500 sm:flex">
                  {template.audience.slice(0, 2).map((item) => (
                    <span key={item} className="rounded-full bg-indigo-100/80 px-3 py-1">
                      {item}
                    </span>
                  ))}
                </div>
