    ...
    <div className="fd-page-chrome mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      <header className="mb-8 flex flex-col gap-4 md:mb-10">
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm">
          <button
            onClick={() => jumpToAny(["diagnose-input-mobile", "diagnose-input-tablet", "diagnose-input-desktop"])}
            className="inline-flex items-center gap-1 font-medium text-indigo-700 transition hover:text-violet-700"
          >
            <span>直接开始输入</span>
            <span aria-hidden>→</span>
          </button>
          <button
            onClick={() => jumpToAny(["diagnose-result-panel"])}
            className="inline-flex items-center gap-1 font-medium text-indigo-700 transition hover:text-violet-700"
          >
            <span>看当前结果</span>
            <span aria-hidden>→</span>
          ...
          <button
            onClick={() => jumpToAny(["diagnose-examples"])}
        ...
        {mobileStep === 2 && (
          <Card className="rounded-2xl border border-slate-200 bg-white py-0 shadow-sm">
            ...
            <CardContent className="grid gap-3 pb-6">
              ...
              <NativeSelect
                value={form.authMode ?? "unknown"}
                onChange={(value) => updateField("authMode", value)}
                options={authModes.map((item) => ({ label: item, value: item }))}
              />
              <NativeSelect
                value={form.transport ?? "auto"}
                onChange={(value) => updateField("transport", value)}
                options={transports.map((item) => ({ label: item, value: item }))}
              />
          ...
          <Card className="rounded-2xl border border-slate-200 bg-white py-0 shadow-sm">
            ...
            <CardContent className="grid gap-3 pb-6">
              ...
              <div className="mt-2 flex flex-wrap gap-3">
                ...
                <Button className="rounded-full" onClick={runDiagnose} disabled={isPending}>
                  开始诊断
                </Button>
              </div>
            </CardContent>
      ...
      <section id="diagnose-examples" className="mt-8 rounded-[28px] border border-slate-200 bg-indigo-50/70 p-4 shadow-sm sm:p-5">
        <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          ...
          <p className="hidden max-w-md text-sm leading-6 text-slate-500 lg:block">
            这层放到后面，是为了先让你直接进入输入动作；只有需要时再借示例起步。
          </p>
        </div>
        <div className="grid gap-3 md:grid-cols-3">
    ...
    <Card className="rounded-3xl border border-slate-200 bg-white py-0 shadow-sm">
      ...
      <CardContent className="space-y-5 pb-6">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4">
          ...
          <div className="mt-3">
            <IssueTypeSelector value={issueTypeValue} onChange={(value) => setForm("issueType", value)} issueTypes={issueTypes} issueTypeMeta={issueTypeMeta} />
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4">
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            ...
            <NativeSelect
              value={form.authMode ?? "unknown"}
              onChange={(value) => setForm("authMode", value)}
              options={authModes.map((item) => ({ label: item, value: item }))}
            />
            <NativeSelect
              value={form.transport ?? "auto"}
              onChange={(value) => setForm("transport", value)}
              options={transports.map((item) => ({ label: item, value: item }))}
            />
          </div>
        </div>
